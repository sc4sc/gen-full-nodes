...
nodeTypes.forEach(nodeType => {
  ...
  const staticNodes = JSON.parse(shell.cat(jsonPath).toString()).map(
    (enode, nodeNum) => {
  ...
  new shell.ShellString(JSON.stringify(staticNodes, undefined, 2)).to(jsonPath);
});
...
range(NODE_COUNT).forEach(n => {
  nodeTypes.forEach(nodeType => {
    ...
    const targetNodeType =
      nodeType === nodeTypes[2] ? nodeTypes[1] : nodeTypes[0];

    shell.cp(
      `./tmp/${targetNodeType}/scripts/static-nodes.json`,
      `${nodeDataDir}/static-nodes.json`
    );
    shell.cp(
      `./tmp/${nodeType}/keys/nodekey${n + 1}`,
      `${nodeDataDir}/klay/nodekey`
    );
    shell.cp(
      `./tmp/${nodeType}/keys/validator${n + 1}`,
      `${nodeDataDir}/klay/validator`
    );
  });
});
...
range(NODE_COUNT).forEach(n => {
  nodeTypes.forEach(nodeType => {
    const nodeDataDir = createNodeDataDir(n, nodeType);
    const cmd = `${KLAYTN} --datadir ${nodeDataDir} init ${nodeDataDir +
      "/genesis.json"}`;
